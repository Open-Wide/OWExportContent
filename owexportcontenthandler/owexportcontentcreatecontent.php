...
class OWExportContentCreateContent extends OWExportContentHandler
{
    ...
    function OWExportContentCreateContent( )
    {
    }
    ...
    function proccessCreateContent( $xmlNode, $parentNodeID = false )
    {
        ...
        foreach ( $objectList as $objectNode )
        {
            ...
            $objectInformation['sectionID'] = $this->getReferenceID( $objectNode->getAttribute( 'sectionID' ) );
            $objectInformation['ownerID'] = $objectNode->getAttribute( 'ownerID' );
            $objectInformation['creatorID'] = $objectNode->getAttribute( 'creatorID' );
            $objectInformation['attributes'] = array();
            $objectInformation['sort_field'] = $objectNode->hasAttribute( 'SortField' ) ? $objectNode->getAttribute( 'SortField' ) : 'path';
            $objectInformation['sort_order'] = $objectNode->hasAttribute( 'SortOrder' ) ? $objectNode->getAttribute( 'SortOrder' ) : 'asc';
            ...
            if ($objectNode->hasChildNodes()) {
            	$childs = $objectNode->childNodes;
	            foreach ($childs as $child) {
	            	if ($childs->nodeName != 'Childs' ) {
		            	$this->proccessCreateContent( $child, $refInfo['node_id'] );
	            	}
	            }
            }
